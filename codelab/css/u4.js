...

    {
      ...
      questions: [
        { q: "A link needs padding and a background like a button, but must stay in the text flow. Which display?",
          ...
          answer: 0, explain: "inline-block = flows like text, sizes like a block. Plain `inline` refuses width and height outright, and its vertical padding paints without pushing the surrounding lines apart, so the background overlaps the text above." },
        { q: "The badge pattern is…",
          code: ".wrap  { position: ???; }\n.badge { position: absolute; top: 8px; right: 8px; }",
          lang: "css",
          choices: ["relative — the parent stays put", "absolute — the card floats free", "sticky — the badge follows the scroll", "static — the browser default"],
          answer: 0, explain: "Absolute children pin to the nearest POSITIONED ancestor, and `position: relative` on `.wrap` makes it that anchor without shifting the card a single pixel. `absolute` would yank the card out of the flow, `sticky` would drag it around on scroll, and `static` (the default) anchors nothing — the badge escapes to the page corner." },
        { q: "You forgot the parent's `position: relative`. Where does the absolute badge go?",
          choices: ["It anchors to the page corner instead", "It disappears from the page entirely", "It falls back to static positioning", "Nothing changes; top/right still apply"],
          answer: 0, explain: "It keeps climbing ancestors until it finds a positioned one — with none anywhere, that's the page itself, so `top: 8px; right: 8px` measures from the document corner instead of the card. Classic bug: the badge is still there, just miles away." },
        { q: "`position: sticky` isn't sticking. Most common cause?",
          choices: ["No top or bottom offset was set", "sticky only works with JavaScript help", "The element is a div, not a header", "The z-index was left off the rule"],
          answer: 0, explain: "sticky requires a threshold like `top: 0` — without one there's nothing to stick TO, so it just scrolls away like a normal element. (The other classic killer: an ancestor with `overflow: hidden`.)" },
        { q: "fixed vs sticky?",
          choices: ["fixed pins always; sticky pins at its offset", "sticky pins to the viewport from the start", "fixed scrolls away once you pass its offset", "fixed only works on images and banners"],
          answer: 0, explain: "A fixed element leaves the flow and locks to the viewport immediately. A sticky one scrolls along normally until it reaches its `top` offset, locks there, then lets go again at the end of its parent. Chat bubbles = fixed; section headers that catch at the top = sticky." },
        { q: "Which element wins the overlap?",
          code: ".a { position: absolute; z-index: 3; }\n.b { position: absolute; z-index: 7; }",
          lang: "css",
          choices: [".b — a higher z-index stacks on top", ".a — the lower z-index sits on top", "Whichever element comes last in the HTML", "They merge into a single blended layer"],
          answer: 0, explain: "Bigger number, higher stack — so `.b`'s 7 beats `.a`'s 3 no matter which one comes first in the HTML. Source order is only the tiebreaker when z-index is equal, and a `static` element ignores z-index altogether — the rule needs `relative`, `absolute`, `fixed` or `sticky` to bite (flex and grid children, which you meet later, are the exception)." },
