...

    {
      ...
      questions: [
        ...
        { q: "What's the difference between `.card p` and `p.card`?",
          choices: [".card p = a p INSIDE .card; p.card = a p that HAS the class card", "They're identical", "p.card is invalid", ".card p is faster"],
          answer: 0, explain: "The space means descendant; no space means both conditions on the same element." },
        { q: "Who wins this fight?",
          code: "p        { color: gray; }\n.note    { color: orange; }\n#special { color: red; }\n/* on: <p id=\"special\" class=\"note\"> */",
          lang: "css",
          choices: ["#special — id beats class beats tag", ".note — classes always win", "p — first rule wins", "They blend together"],
          answer: 0, explain: "Specificity ladder: id > class > tag. Position only breaks ties." },
        { q: "Two rules have EQUAL specificity. Which applies?",
          choices: ["The one written later in the stylesheet", "The one written first", "Neither", "The shorter one"],
          answer: 0, explain: "Last-one-wins on ties — that's the cascade's tiebreaker." },
        { q: "Which properties inherit from `body`?",
          choices: ["Text properties like color and font-family", "Borders and padding", "Backgrounds", "All properties"],
          answer: 0, explain: "Text styles flow down; box-model properties would be chaos if they inherited (imagine every element bordered!)." },
        { q: "`li:first-child` selects…",
          choices: ["An li that is the first child of its parent", "The first li's child", "Every li's first word", "The li with class first-child"],
          answer: 0, explain: "Pseudo-classes (with a colon) select by position or state — :last-child and :hover work the same way." },
        { q: "The professional default for adding CSS to a multi-page site?",
          choices: ["An external stylesheet via <link>", "style= attributes everywhere", "A <style> block copied into every page", "JavaScript"],
          answer: 0, explain: "One external file, many pages, one place to change things. Inline styles are the last resort." },
        { q: "A style \"mysteriously\" isn't applying. Most likely culprit?",
          choices: ["A more specific rule elsewhere is overriding yours", "The browser hates you", "CSS caches forever", "You must reboot"],
          answer: 0, explain: "First debugging instinct: who else targets this element, and do they outrank me?" }
      ]
    }
  ]
});
