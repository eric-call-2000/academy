...

    {
      ...
      questions: [
        { q: "What can localStorage actually store?",
          choices: ["Any JavaScript value, arrays and functions included", "Numbers and strings, but never booleans", "Only strings — objects must travel as JSON", "Anything, but it clears when the tab closes"],
          answer: 2, explain: "setItem coerces **everything** to a string, so objects and arrays have to ride along as JSON: `JSON.stringify(tasks)` on the way in, `JSON.parse(...)` on the way out. Numbers come back as `\"3\"`, not `3`. And it is not session-scoped — the data survives closing the tab and reloads just fine." },
        { q: "In the state-driven pattern, what is the ONLY job of an event handler?",
          choices: ["Build the new <li> by hand and append it to the list", "Change the state (and save), then call render()", "Reload the page so the list refreshes", "Edit index.html so the new task is in the markup"],
        ...
        { q: "Why the `|| []` at the end?",
          ...
          lang: "js",
          choices: ["It merges stored data with an empty array", "JSON.parse is slow, so [] acts as a cache", "It wipes storage clean on every page load", "JSON.parse(null) returns null, not an array"],
          answer: 3, explain: "On a first visit nothing is stored yet, so `getItem` hands back `null` and `JSON.parse(null)` is also `null`. The `|| []` swaps that null for an empty array — without it, `tasks` would be null and the very first `tasks.push` would explode." },
        { q: "Why does the submit handler start with e.preventDefault()?",
          choices: ["Forms reload the page on submit by default", "It stops the user from typing in the input", "It saves the form to localStorage automatically", "It's decoration; nothing changes without it"],
          answer: 0, explain: "A default form submit navigates away and reloads the document, throwing your in-memory `tasks` array out with it — the app blinks and forgets. `preventDefault()` cancels that navigation and turns the form into a pure JavaScript event source, so your state (and your app) stays alive." },
        { q: "The Open filter is active, hiding 3 done tasks — yet the counter must still say how many are open. Where do the list and the counter each get their numbers?",
          choices: ["Both from the DOM — count the <li> elements", "The counter from the DOM, the list from state", "Both are derived from the tasks array in state", "The counter needs its own separate count variable"],
          answer: 2, explain: "The DOM is just a **picture** of state, never the source of it. `render()` derives a `visible` slice for display and counts open tasks from the FULL array, so a filter can hide rows without lying about the count. Reading numbers back out of the DOM — or keeping a second tally beside the array — is exactly how apps drift out of sync." },
