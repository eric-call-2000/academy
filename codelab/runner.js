...
(function () {
  ...
  function runShell(lesson, files, hooks) {
    ...
    return new Promise(function (resolve) {
      ...
      var fsTree, result, GIT = window.CODELAB.git, DOCKER = window.CODELAB.docker, CI = window.CODELAB.ci, before = null;
      ...
      try {
        ...
        /* cisim's snapshot composes docker's (which composes git's), so one
           call covers all three; fall back down the chain if a layer is absent. */
        if (CI) before = CI.snapshot(fsTree);
      ...
      /* CI helpers (T.run, T.deployed, T.merged, T.artifact …) merge in too. */
      if (CI) CI.extendT(T, fsTree, before);
