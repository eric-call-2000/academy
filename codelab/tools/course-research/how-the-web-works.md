...
- Concept picks and quizzes rebalanced so the correct answer is never the longest choice (0/17 concept picks, 0/18 quiz questions). The `mock`/`mockFn` fetch harness is *not* used — cache and conditional-request logic is graded as pure functions over plain request/response objects, which is exact and clock-controlled, where a static mock could not model a resource changing over time.
