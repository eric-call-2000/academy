...
**Tranche B as built (2026-09-16).** Units 4-6 (the HTTP message): U4 methods/status/redirects (js classifyMethod: isSafe/isIdempotent/canRetryAutomatically per RFC 9110 — GET/HEAD safe+idempotent, PUT/DELETE idempotent-not-safe, POST/PATCH neither), U5 headers/content (all concept: the envelope, content negotiation + Vary, gzip/Brotli, Content-Length/ranges/206), U6 caching (js cacheState with lesson.clock: fresh/stale/revalidate from max-age; js conditionalGet: ETag match -> 304 with 0 bytes transferred; concept no-store vs no-cache vs must-revalidate, browser/CDN layers). Course now 30 items, ~5.5h -> **3 credits {fnd 3}** (from 1); no new engine at all (reuses the clock and the tranche-A gates). All facts node-verified (method properties, cacheState boundary, conditionalGet). Quiz answers rebalanced to 0/35 longest. The `mock`/`mockFn` fetch harness is *not* used — cache and conditional-request logic is graded as pure functions over plain request/response objects, which is exact and clock-controlled, where a static mock could not model a resource changing over time.

**Tranche C as built (2026-09-17) — COURSE COMPLETE.** Units 7-8. U7 connections/versions (4 concept lessons + quiz): HTTP/1.1 one-at-a-time + ~6-connection limit and head-of-line blocking (reuses the `waterfall` lab: 12 resources / 6 connections, predict `ceil(12/6)`); HTTP/2 multiplexing (waterfall lab, 12 streams on one connection -> 1 round) with the "removes ALL head-of-line blocking" misconception screen (TCP packet loss still stalls every stream); HTTP/3 over QUIC (independent transport streams, folded TLS handshake -> 1-RT setup); and a build-advice lesson (bundling was an HTTP/1.1 fix; over-bundling busts the whole cache — ties U3 round trips + U6 hashed filenames). U8 (2 projects + final quiz): web-u8-p1 concept capstone (order the cold-request stages, trace the 4 round trips, 200 vs 304, HTTP/2 sub-resource fetch, plain-English explain, rendering handoff to the frontend courses); web-u8-p2 js page-load analyser with `lesson.clock` (isFresh/analyze/roundTrips/pageLoad — cold HTTP/1.1 `ceil(14/6)`=3 rows vs HTTP/2 1 row; warm = 12 revalidations at 0 bytes but still 2 rows; caching fix -> 0 requests/0 rows; all counts node-verified first). **NO new engine** (reuses the waterfall lab, `lesson.clock`, and the tranche-A/B gates). Course FINAL: **38 items, ~7.6h -> 4 credits {fnd 4}**, `targetHours` dropped. Concept picks rebalanced course-wide from 50% -> 32% answer-is-longest (U7/U8 were 10/10 strictly-longest; lengthened one distractor each). Full `npm run validate` ALL PASSED (0 failures); catalog 84 credits built, 6/7 positions reachable. **Remaining: make `web` required on fe/fs/be** (raises those sheets' required-credit sums + `total` by 4 each — a threshold change left for Eric to approve).
