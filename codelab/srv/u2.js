...

    {
      ...
      questions: [
        { q: "Your API stores records in `let todos = []`. What happens to them when the server process restarts?",
          choices: ["They are saved to disk automatically", "The browser keeps a backup copy", "They vanish — in-memory data dies with the process", "They move to localStorage"],
          answer: 2, explain: "An array lives in RAM; restart = blank slate. Real backends add PostgreSQL/MongoDB for durability — the routes stay the same, only the store changes." },
        ...
        { q: "The real difference between PUT and PATCH?",
          choices: ["PUT is only for creating records", "PUT replaces the whole record; PATCH merges partial changes", "PATCH replaces; PUT merges", "No difference — pick whichever reads better"],
          answer: 1, explain: "Fields missing from a PUT body are deleted from the record; PATCH only touches the fields you send. Mixing them up silently erases data." },
        { q: "`Number(\"abc\")` evaluates to…",
          choices: ["0", "null", "NaN — detect it with Number.isNaN(...)", "It throws a TypeError"],
          answer: 2, explain: "Number never throws; junk becomes NaN. And since NaN !== NaN, you must test with Number.isNaN — which is why parsePath converts NaN to a friendly null." },
