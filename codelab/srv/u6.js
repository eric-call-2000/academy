...

    {
      ...
      questions: [
        { q: "`validateSignup(body)` collects problems. What should it return for a perfectly VALID body?",
          choices: ["null for no problems", "true, since it passed", "An empty array", "{ status: 200 }"],
          answer: 2, explain: "One return type for every outcome: an array — empty means zero problems were found. No null, no boolean, no status object mixed in, so the caller never has to type-check the result and just asks `problems.length === 0`." },
        { q: "A client signs up with a 5-character password. Which status is right?",
          choices: ["400 — the CLIENT sent bad data", "500 — the server failed to save it", "201 — create it and warn them", "301 — redirect them to a longer form"],
          answer: 0, explain: "4xx = the client's mistake, 5xx = yours. A five-character password is invalid data the client sent, so 400 is the honest answer — nothing on your side failed, nothing was created, and a redirect just hides the real problem." },
        { q: "Why give EVERY failure the same `{ error: { code, message } }` envelope?",
          choices: ["It makes responses smaller on the wire", "One client code path handles any failure", "HTTP requires exactly that shape", "It hides the status code from the user"],
          answer: 1, explain: "One predictable envelope means the client handles ANY failure with a single code path: machines branch on `error.code`, humans read `error.message`, and the front-end writes its failure handler exactly once. HTTP itself doesn't mandate any body shape, and the status code still travels normally — the consistency is a gift you give your callers." },
        { q: "What does safeHandler's try/catch actually buy you when a route throws?",
          code: "try { return handler(req); }\ncatch (err) { /* … */ }",
          lang: "js",
          choices: ["The bug quietly fixes itself", "The request is retried automatically", "The error is silently ignored and 200 goes out", "A clean 500 instead of a dead server"],
          answer: 3, explain: "The catch turns a thrown error into a clean 500 envelope instead of a dead process — one bad request no longer takes every other user down with it. Nothing gets retried, and nothing gets buried behind a 200. The bug is still yours to fix; the try/catch just buys you a server that's still alive to fix it on." },
        { q: "7 items, perPage = 3. How many pages?",
          choices: ["2", "3", "2.33", "4"],
          answer: 1, explain: "`Math.ceil(7 / 3)` = 3. The 1-item leftover still needs a page of its own — that's why it's ceil, never floor (which would drop those items on the floor) and never the raw 2.33. Pages are whole things." },
        { q: "Which start index serves page 2 when perPage is 3?",
          code: "const items = all.slice(start, start + perPage);",
          lang: "js",
          choices: ["start = 1", "start = 2", "start = 3", "start = 6"],
          answer: 2, explain: "The formula is `(page - 1) * perPage`, so `(2 - 1) * 3 = 3`: page 2 begins exactly where page 1's three items ended. Using the raw page number would land on item 2, and `page * perPage` would skip a whole page." }
      ]
    }
  ]
});
