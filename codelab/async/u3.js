...

    {
      ...
      questions: [
        { q: "Your fetch hits a 404 Not Found. What happens?",
          ...
          lang: "js",
          choices: ["The await throws and your catch block runs", "The promise resolves normally — you must check res.ok yourself", "res is undefined", "fetch retries automatically until the endpoint exists"],
          answer: 1, explain: "fetch only rejects on NETWORK failure (offline, DNS, CORS). An HTTP error is a successful conversation with bad news — res.ok and res.status are your job." },
        ...
        { q: "flaky() rejects, but \"saved!\" never prints. Why?",
          code: "try {\n  flaky(); // ← no await\n} catch (err) {\n  console.log(\"saved!\");\n}",
          lang: "js",
          choices: ["Without await, the rejection happens later — after try/catch has already finished", "try/catch is not allowed inside async functions", "The parameter must be called error, not err", "console.log cannot run inside catch blocks"],
          answer: 0, explain: "try/catch only sees exceptions thrown while it runs. Awaiting the promise re-throws the rejection INSIDE the try — that's the whole trick." },
        { q: "In this chain, when does showError run?",
          code: "loadUser()\n  .then(render)\n  .catch(showError);",
          lang: "js",
          choices: ["Only if loadUser() rejects", "Never — .catch only works with try blocks", "After every run, success or not", "If loadUser() rejects OR render throws — .catch hears failures from anywhere above it"],
          answer: 3, explain: "A .catch at the end of a chain is a safety net for every link above it — that's why it usually goes last." },
        { q: "What does calling f() give you?",
          code: "async function f() {\n  throw new Error(\"nope\");\n}",
          lang: "js",
          choices: ["undefined", "It crashes the program the moment f is defined", "A promise that rejects with that Error", "The string \"nope\""],
        ...
        { q: "The request failed and won't recover. What should your UI do?",
          choices: ["Replace \"Loading…\" with a short, human error message (plus a retry button if you can)", "Keep showing \"Loading…\" — maybe it will fix itself", "Print the raw stack trace into the page", "Nothing — errors belong in the console"],
          answer: 0, explain: "A spinner that never ends is the worst UX in the catalog. Tell users what happened in their language, and offer a way forward." }
      ]
    }
  ]
});
