...

    {
      ...
      questions: [
        { q: "The heart fills the instant you tap ❤️ — the request is still in flight. What pattern is that?",
          choices: ["Lazy loading", "Server-side rendering", "Optimistic UI — update the page first, roll back if the server says no", "Debouncing"],
          answer: 2, explain: "Optimistic UI bets on success for an instant feel — and the rollback branch keeps it honest when the server fails." },
        { q: "The server answers 404. What does this line do?",
          code: "const res = await fetch(\"/api/nope\");",
          lang: "js",
          choices: ["Resolves normally — res.ok is false and res.status is 404", "Throws, jumping straight to catch", "res is null", "Retries automatically until it gets a 200"],
        ...
        { q: "Which call correctly POSTs the object `data` as JSON?",
          choices: ["fetch(url, { body: data })", "fetch(url, \"POST\", data)", "fetch(url, { method: \"POST\", body: data })", "fetch(url, { method: \"POST\", headers: { \"Content-Type\": \"application/json\" }, body: JSON.stringify(data) })"],
          answer: 3, explain: "Three parts every time: the method, the Content-Type header, and a STRINGIFIED body — fetch does not stringify objects for you." },
        { q: "You need two independent endpoints, and you want the data as fast as possible. Best tool?",
          choices: ["await the first, then await the second", "Promise.all — both requests run at the same time", "A setInterval that polls each one", "Nest one fetch inside the other's .then"],
        ...
        { q: "Your search box re-filters on every keystroke — the page repaints 9 times while typing 'parasite'. The classic fix?",
          choices: ["Cache the keystrokes in localStorage", "Make the filter loop faster", "Debounce — each keystroke resets a short setTimeout, so the filter runs once after typing pauses", "Only allow one-letter searches"],
          answer: 2, explain: "clearTimeout + setTimeout on every input event means the real work happens once, ~150 ms after the last key." },
        { q: "What lands in `result`?",
          code: "async function getRating() {\n  const res = await fetch(\"/api/movie\");\n  const data = await res.json();\n  return data.rating;\n}\nconst result = getRating();",
          lang: "js",
          choices: ["The rating number", "A Promise — you still need await getRating() (or .then) to get the number", "undefined", "The raw JSON text"],
          answer: 1, explain: "async functions ALWAYS return a promise, even when the body returns a plain value — that contract never bends." },
