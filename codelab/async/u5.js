...

    {
      ...
      questions: [
        { q: "You need data from two INDEPENDENT endpoints. The fastest correct approach?",
          choices: ["await fetch(a); await fetch(b); — one after the other", "Promise.race([fetch(a), fetch(b)]) — take whichever wins", "await Promise.all([fetch(a), fetch(b)]) — both at once", "Wrap both fetches in setTimeout(…, 0)"],
        ...
        { q: "What is `a`?",
          ...
          lang: "js",
          choices: ["\"A\" — results match the order you passed the promises in", "\"B\" — whichever finishes first comes first", "undefined — the slow one gets dropped", "It's random on every run"],
          answer: 0, explain: "Promise.all lines results up with the INPUT order, no matter who finished first. That's what makes destructuring safe." },
        ...
        { q: "A search box fires a request on every keystroke. Debouncing it at 150 ms means…",
          choices: ["Each request is delayed 150 ms, but ALL of them still fire", "The search runs ONCE, 150 ms after the last keystroke of a burst", "The search runs every 150 ms while the user types", "Keystrokes during the first 150 ms are thrown away"],
          answer: 1, explain: "Every keystroke clears the pending timer and sets a new one — only 150 ms of silence lets the search fire. (Running every 150 ms WHILE typing is the cousin pattern: throttling.)" },
        { q: "Roughly how long does this take?",
          code: "const a = await wait(200, \"A\");\nconst b = await wait(200, \"B\");",
          lang: "js",
          choices: ["~200 ms — they run in parallel automatically", "~0 ms — awaits are free", "~400 ms — the second wait doesn't even START until the first finishes", "~800 ms — each await doubles the delay"],
          answer: 2, explain: "await pauses the function, so the timers run back-to-back: 200 + 200. Start both promises first (or use Promise.all) to overlap them into ~200 ms." },
        { q: "What does Promise.race([p1, p2]) settle with?",
          choices: ["An array of both results, fastest first", "The result of whichever promise settles FIRST — resolve OR reject", "Always the first promise in the array", "The fastest resolved value — rejections are ignored"],
