...

    {
      ...
      questions: [
        { q: "You're adding a brand-new note to the server. Which HTTP method is the conventional choice?",
          choices: ["GET — it gets the note to the server", "PUT — you're putting a note there", "POST — create something new on the server", "DELETE — clears space for the new note"],
        ...
        { q: "What does `JSON.stringify` do in this request?",
          ...
          lang: "js",
          choices: ["Converts the object into JSON text, because HTTP bodies travel as text", "Encrypts the object so only the server can read it", "Parses the server's response into an object", "Makes the request synchronous"],
          answer: 0, explain: "Request bodies are strings on the wire. stringify turns your object into JSON text; the server parses it back on its side." },
        { q: "Why send the header `\"Content-Type\": \"application/json\"` with a POST?",
          choices: ["It makes the response arrive faster", "It's required on every GET request too", "It converts the body to JSON automatically", "It tells the server how to read the body: \"what I'm sending you is JSON\""],
          answer: 3, explain: "Headers describe the request. Without it, many servers won't even try to parse your JSON body." },
        { q: "Your POST comes back with status **201**. What is the server saying?",
          choices: ["Try again later — it's busy", "Created — the new resource was saved", "Moved — the URL has changed", "Unauthorized — log in first"],
        ...
        { q: "After this request, `res.status` is 400. What just happened?",
          ...
          lang: "js",
          choices: ["fetch threw an error, so this line never finished", "The network connection is down", "The server received the request and rejected the DATA — fetch resolves normally, so you must check res.ok yourself", "The browser will retry the request automatically"],
          answer: 2, explain: "fetch only rejects on network failure. A 400 is a perfectly successful conversation with an unhappy server — branching on res.ok is your job." },
