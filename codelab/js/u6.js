...
window.CODELAB.addUnit("js", {
  ...
  lessons: [
...

    {
      ...
      questions: [
        { q: "What is `\"5\" + 1`?",
          choices: ["\"51\"", "6", "NaN", "The number 51"],
          answer: 0, explain: "`+` is overloaded: if either side is a string it CONCATENATES instead of adding, so \"5\" + 1 glues into the **string** \"51\" — the #1 form-input bug in existence, since every input value arrives as text. Convert with `Number()` first when you want arithmetic." },
        { q: "`\"hello world\".slice(0, 5)` returns…",
          choices: ["\"hello\"", "\"hello \"", "\"world\"", "\"hell\""],
          answer: 0, explain: "`slice(start, end)` cuts from start UP TO but NOT including end — indexes 0, 1, 2, 3 and 4 give you \"hello\", and the space living at index 5 stays behind. The count of characters you get is simply end minus start." },
        { q: "Which cleans `\"  Ada Lovelace \"` into `\"ada_lovelace\"`?",
          code: "A) raw.trim().toLowerCase().replaceAll(\" \", \"_\")\nB) raw.toLowerCase().replaceAll(\" \", \"_\").trim()",
          lang: "js",
          choices: ["A — trim first, before spaces become underscores", "B — lowercasing first is what lets trim work", "B — trim belongs at the end of a chain", "Chain order does not change the result"],
          answer: 0, explain: "Chain order matters enormously. B lowercases and replaces FIRST, so all three padding spaces — the two in front and the one at the end — become underscores, and `trim()` only strips whitespace, so it can no longer touch them: B hands back `__ada_lovelace_`. A trims while that padding is still whitespace, so the replace only ever meets the single space in the middle: `ada_lovelace`." },
        { q: "`Math.floor(7.9)` vs `Math.round(7.9)`?",
          choices: ["7 and 8", "8 and 8", "7 and 7", "8 and 7"],
          answer: 0, explain: "`Math.floor` always drops DOWN ⬇ (7.9 → 7), `Math.ceil` always pushes UP ⬆, and `Math.round` goes to the CLOSEST (7.9 → 8). floor is not \"round down when below .5\" — it heads for the lower integer every single time." },
        { q: "What does `(2.5).toFixed(2)` give you?",
          choices: ["The STRING \"2.50\"", "The number 2.5", "The STRING \"2.5\", no padding", "A TypeError at runtime"],
          answer: 0, explain: "`toFixed` formats for DISPLAY and hands back a string, never a number. It also emits exactly as many decimals as you asked for, padding with zeros to get there, so `(2.5).toFixed(2)` is \"2.50\" and not \"2.5\". Do all your math first and call toFixed at the very last moment; feeding its result back into arithmetic is how `\"2.50\" + 1` becomes \"2.501\"." },
        { q: "`parseInt(\"42px\")` returns…",
          choices: ["42", "NaN", "\"42\"", "0"],
          answer: 0, explain: "`parseInt` reads the LEADING digits and stops at the first character that isn't one, handing back a real number — perfect for CSS values like \"42px\". `Number(\"42px\")` gives NaN instead, because for Number the WHOLE string has to convert." },
        { q: "The reliable way to check \"did my conversion fail\"?",
          choices: ["Number.isNaN(Number(text))", "Number(text) === NaN", "typeof text === \"NaN\"", "text.isNaN() directly"],
          answer: 0, explain: "NaN is the one value in JavaScript that doesn't even equal itself, so any `=== NaN` check is always false. `typeof NaN` is \"number\", never \"NaN\". And strings have no `.isNaN()` method of their own. Convert first, then test the result with `Number.isNaN(...)`." }
      ]
    },
