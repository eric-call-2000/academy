...

    {
      ...
      questions: [
        { q: "In an else-if chain, which branch runs?",
          choices: ["The FIRST one whose condition is true — the rest are skipped", "Every branch whose condition is true", "The last true one", "All of them"],
          answer: 0, explain: "Top-down, first-match-wins. That's why you order checks from strictest to loosest." },
        { q: "What does `gradeFor(95)` return here?",
          code: "function gradeFor(s) {\n  if (s >= 70) return \"C\";\n  else if (s >= 80) return \"B\";\n  else if (s >= 90) return \"A\";\n  return \"F\";\n}",
          lang: "js",
          choices: ["\"C\" — the loosest check is first and wins every time", "\"A\"", "\"B\"", "\"F\""],
          answer: 0, explain: "95 >= 70 is true, so it returns C immediately. Order the chain strictest-first!" },
        { q: "`a && b` is true when…",
        ...
        { q: "When is `switch` nicer than an else-if chain?",
          choices: ["Matching ONE value against many exact options", "Comparing ranges like score >= 90", "Combining several different variables", "Never"],
          answer: 0, explain: "switch does strict equality on a single value — menus, commands, animal sounds. Ranges still want if/else." },
        { q: "What does this evaluate to?",
          code: "const label = 0 ? \"yes\" : \"no\";",
          lang: "js",
          choices: ["\"no\" — 0 is falsy", "\"yes\"", "0", "undefined"],
          answer: 0, explain: "Ternary: condition ? ifTrue : ifFalse. Zero is one of the falsy six." },
        { q: "Which are ALL of the falsy values?",
          choices: ["false, 0, \"\", null, undefined, NaN", "false, 0, \"\", [], {}, null", "false and null only", "Anything in quotes"],
          answer: 0, explain: "The falsy six. Note that [] and {} are TRUTHY — a classic gotcha." },
        { q: "A user legitimately has 0 items in their cart. Which default is safe?",
          ...
          lang: "js",
          choices: ["Both give 0 here — but A (??) is safer because || would ALSO wipe out a real 0 elsewhere", "B is always safer", "Neither works with numbers", "?? only works on strings"],
          answer: 0, explain: "?? treats only null/undefined as missing; || treats every falsy (including a genuine 0 or \"\") as missing." }
      ]
    },
