...

    {
      ...
      questions: [
        { q: "What does `brew()` return?",
          ...
          lang: "js",
          choices: ["\"medium coffee\" — both defaults kick in", "\"undefined undefined\"", "An error", "\"coffee medium\""],
          answer: 0, explain: "Missing arguments trigger their defaults, left to right." },
        { q: "Why do developers love guard clauses?",
          choices: ["Bad input exits early, so the happy path stays flat and readable", "They run faster", "They're required by strict mode", "They replace all if statements"],
          answer: 0, explain: "Validate-and-bail at the top beats wrapping the whole function in nested ifs." },
        { q: "A callback is…",
          choices: ["A function passed into another function to be called there", "A function that calls itself", "A returned string", "A loop"],
          answer: 0, explain: "Functions are values. `.map(n => n * 2)` passes that arrow as a callback." },
        { q: "What prints?",
          code: "function makeCounter() {\n  let count = 0;\n  return () => ++count;\n}\nconst a = makeCounter();\nconst b = makeCounter();\na(); a();\nconsole.log(a(), b());",
          lang: "js",
          choices: ["3 1 — each closure owns a private count", "3 3", "1 1", "2 2"],
          answer: 0, explain: "Every makeCounter() call creates a fresh count; a's history never leaks into b." },
        { q: "Which line is the PURE way to add to an array parameter?",
          code: "A) list.push(item); return list;\nB) return [...list, item];",
          lang: "js",
          choices: ["B — copy, don't mutate the caller's array", "A — push is idiomatic", "Both are pure", "Neither works"],
          answer: 0, explain: "A mutates an array the caller still holds — a classic source of spooky bugs. Spread builds a new one." },
        { q: "A pure function may…",
          choices: ["Read its parameters and return a value — nothing else", "Modify globals if it's careful", "console.log freely and still be pure", "Mutate its arguments"],
          answer: 0, explain: "Same input → same output, zero side effects. (Logging is technically a side effect too — fine for learning, but know it.)" }
      ]
    },
