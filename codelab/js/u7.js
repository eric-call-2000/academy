...

    {
      ...
      questions: [
        { q: "Inside a method, `this` refers to…",
          choices: ["The object the method was called on", "The global window object, always", "The function object itself", "The nearest enclosing variable"],
          answer: 0, explain: "`player.addPoints(5)` → inside addPoints, `this` IS player: whatever sits to the LEFT of the dot at call time. That is what keeps data and behavior together in one object. It is never the function itself, and it is only `window` when there is no object in front of the call." },
        { q: "What does `order?.customer?.city` do when customer is missing?",
          choices: ["Evaluates to undefined instead of crashing", "Throws a TypeError you have to catch", "Quietly returns null for the rest of the chain", "Creates an empty customer object first"],
          answer: 0, explain: "Each `?.` short-circuits: the moment a link in the chain is null or undefined, the whole expression stops and evaluates to **undefined** — no crash, no null, and nothing gets created along the way. Pair it with `?? \"Unknown\"` to supply a fallback and finish the job." },
        { q: "Which converts an object so you can `.reduce` over its numbers?",
          choices: ["Object.values(obj)", "obj.toArray()", "obj.reduce(fn, 0)", "JSON.parse(obj)"],
          answer: 0, explain: "`Object.keys` → the names, `Object.values` → the data, `Object.entries` → both as [key, value] pairs. Once you have a real array, every array method applies. Plain objects have neither a `.toArray()` nor a `.reduce()` of their own." },
        { q: "What does this comparator produce?",
          code: "players.sort((a, b) => b.score - a.score)",
          lang: "js",
          choices: ["Highest score first (descending)", "Lowest score first (ascending)", "Alphabetical by player name", "The original order, unchanged"],
          answer: 0, explain: "A comparator returns a POSITIVE number when b should come first. `b.score - a.score` is positive whenever b outscores a, so the biggest scores bubble to the front — descending. Flip it to `a.score - b.score` and you get ascending." },
        { q: "Why copy before sorting (`[...players].sort(…)`)?",
          choices: ["`.sort()` reorders the original array in place", "Sorting a copy runs measurably faster than sorting in place", "`.sort()` refuses to run on an original array", "Spreading is required before any array method"],
          answer: 0, explain: "One of the classic JavaScript betrayals: `.sort()` (like `.reverse()`, `.splice()` and `.push()`) MUTATES the array in place and hands back that same array. Spread-copy first — `[...players].sort(…)` — and the original order survives for everything else that still needs it." },
        { q: "What prints?",
