  ...
  lessons: [
...

    {
      ...
      questions: [
        ...
        { q: "Which builds the greeting `Hi, Ada!` with a template literal?",
          code: "const who = \"Ada\";",
          lang: "js",
          choices: ["`Hi, ${who}!`", "\"Hi, ${who}!\"", "'Hi, + who + !'", "`Hi, {who}!`"],
          answer: 0, explain: "A template literal is wrapped in BACKTICKS, and only inside those does `${who}` act as a slot the value drops into — `who` holds \"Ada\", so the result is \"Hi, Ada!\". Put the same text in \"double\" or 'single' quotes and `${who}` is printed literally, character for character. The `$` carries its weight too: `{who}` on its own is plain text." },
        { q: "What's the difference between `=` and `===`?",
          choices: ["`=` assigns a value; `===` compares two values", "`=` compares loosely; `===` compares strictly", "`===` assigns and compares in a single step", "`===` only compares numbers; `=` handles the rest"],
          answer: 0, explain: "One `=` ASSIGNS — it puts a value into a variable. Three `===` COMPARE strictly: same value **and** same type. That strictness is why `===` beats `==`, the loose version that quietly converts types so `\"5\" == 5` comes out true. No operator does both jobs at once." },
