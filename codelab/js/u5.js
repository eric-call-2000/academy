...

    {
      ...
      questions: [
        { q: "When is `while` the right loop?",
          choices: ["When you know the STOPPING CONDITION, not the count", "When you already know how many laps you need", "When you need to walk every item in an array", "Only when you deliberately want it endless"],
          answer: 0, explain: "\"Keep going until the battery dies\" = `while`. \"Do this 10 times\" = `for`. If you can count the laps up front a `for` loop is clearer; if all you can describe is the condition that ends it, `while` is the honest choice. Neither one has replaced the other." },
        { q: "What's wrong here?",
          code: "let n = 10;\nwhile (n > 0) {\n  console.log(n);\n}",
          lang: "js",
          choices: ["n never changes, so the loop never ends", "`while` needs a semicolon after its condition", "n should be declared with const, not let", "The condition should be `n >= 0` instead"],
          answer: 0, explain: "The body must move the condition toward false — add `n--` (or `n -= 1`) and it counts down and stops. Without that it is an infinite loop that freezes the tab. Everything else here is fine: `let` is right precisely because n has to change, the syntax is correct, and `n >= 0` would spin forever too." },
        { q: "`for…of` vs `for…in`?",
          choices: ["of loops an array's VALUES; in loops an object's KEYS", "in loops an array's VALUES; of loops a plain object's KEYS", "of only works on numbers; in works on any collection", "of is the modern spelling; in is the deprecated one"],
          answer: 0, explain: "Values OF a list, keys IN an object — the mnemonic that sticks. `for…of` walks an array's values (and strings, Maps and Sets); `for…in` walks an object's property NAMES, which you then use to look the values up. Neither is deprecated and neither is limited to numbers." },
        { q: "What does this sum to?",
          code: "let t = 0;\nfor (const n of [5, -2, 0, 9]) {\n  if (n === 0) break;\n  if (n < 0) continue;\n  t += n;\n}",
          lang: "js",
          choices: ["5", "12", "14", "3"],
          answer: 0, explain: "`continue` skips just that one item, so the -2 is never added to t. `break` abandons the whole loop the instant n === 0, so the 9 sitting beyond it is never even looked at. Only the 5 makes it in." },
        { q: "How many times does the inner body run?",
          code: "for (let r = 0; r < 3; r++) {\n  for (let c = 0; c < 4; c++) {\n    visit(r, c);\n  }\n}",
          lang: "js",
          choices: ["12 times", "7 times", "3 times", "4 times"],
          answer: 0, explain: "The inner loop completes fully — all 4 laps — for EACH of the 3 outer laps: rows × cols = 3 × 4 = 12. Nested loops MULTIPLY, they don't add, which is why nesting a loop inside a loop over a big list gets expensive fast." },
        { q: "The \"accumulator pattern\" is…",
          choices: ["Start with an empty result and fold each item into it", "A built-in JavaScript keyword that totals an array for you", "Declare the running total inside the loop body", "Run two separate loops side by side over one array"],
          answer: 0, explain: "`let total = 0` OUTSIDE the loop, `total += n` inside, then use the result after — start empty, fold each item in. Sum, longest word, count, a collected array: one pattern, endless uses. Declaring the total INSIDE the loop resets it every lap, which is the classic bug." }
      ]
    },
