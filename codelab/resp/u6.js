...

    {
      ...
      questions: [
        { q: "A flex container has `flex-direction: column`. Which property now controls the VERTICAL distribution of its children?",
          choices: ["align-items — the cross axis is vertical here", "text-align — it aligns everything vertically", "justify-content — it follows the main axis", "vertical-align — built for vertical spacing"],
          answer: 2, explain: "justify-content always works along the MAIN axis. flex-direction: column rotates that axis, so justify-content now spaces children vertically and align-items takes over the horizontal (cross) axis." },
        { q: "The row gets squeezed. What happens?",
          code: ".side { width: 220px; flex-shrink: 0; }\n.content { flex: 1; }",
          lang: "css",
          choices: ["The sidebar stays exactly 220px; .content shrinks", "Both shrink equally as the row narrows", ".content collapses to zero before the sidebar", "The browser reports an error and ignores both"],
          answer: 0, explain: "flex-shrink: 0 exempts the sidebar from shrinking so it holds exactly 220px, and flex: 1 makes .content the stretchy part that absorbs all the growing AND shrinking — the classic fixed-plus-fluid pair." },
        { q: "What does `grid-template-columns: repeat(auto-fit, minmax(150px, 1fr))` buy you?",
          choices: ["Exactly 150 equal columns, always", "Columns only on screens wider than 150px", "It still needs one media query per breakpoint", "As many ≥150px columns as the row fits"],
          answer: 3, explain: "auto-fit recalculates the column count at every width, and minmax gives each column a floor (150px) and a stretch (1fr) so they fill the row edge to edge — responsive with zero media queries." },
        ...
        { q: "In a mobile-first stylesheet, which rules sit OUTSIDE every media query?",
          choices: ["The desktop layout — phones get max-width queries instead", "Only colors and fonts — all layout belongs in a query", "The small-screen defaults — min-width queries add on", "None — every rule lives inside some media query"],
          answer: 2, explain: "The unqueried rules ARE the phone design, layout included; each @media (min-width: …) block then layers the wider version on top. That order keeps phones — most of your traffic — on the simplest CSS." },
        { q: "What does the middle value do?",
          code: "h1 { font-size: clamp(1.2rem, 4vw, 2rem); }",
          lang: "css",
          choices: ["It is a fallback for browsers without clamp", "It is the preferred size — 4% of viewport width", "It sets a 4-second transition on the font size", "It only applies inside a media query block"],
          answer: 1, explain: "clamp(min, preferred, max): the browser uses the middle value, 4vw, so the heading scales fluidly with the viewport — but it never lets that drop below 1.2rem or grow past 2rem." },
        { q: "Why does nearly every responsive stylesheet include `img { max-width: 100%; height: auto; }`?",
          choices: ["It makes images download faster on slow links", "It crops every image down to a square thumbnail", "Images will not display at all without it", "The image can never overflow its container"],
          answer: 3, explain: "max-width: 100% caps the image at its container's width so it can never spill out; height: auto then lets the height follow along, keeping the aspect ratio while the picture scales down instead of distorting." }
      ]
    }
  ]
});
