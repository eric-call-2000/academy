...

    {
      ...
      questions: [
        { q: "What does `1fr` mean in `grid-template-columns: repeat(3, 1fr)`?",
          choices: ["One frame per second", "1% of the screen width", "One fraction of the free space — three 1fr columns split it into equal thirds", "The first row of the grid"],
          answer: 2, explain: "fr units divide the leftover space proportionally; repeat(3, 1fr) is shorthand for 1fr 1fr 1fr." },
        { q: "In a 3-column grid, which columns does this tile cover?",
          code: ".tile {\n  grid-column: 1 / 3;\n}",
          lang: "css",
          choices: ["Columns 1, 2 and 3", "Columns 1 and 2 — the numbers are grid LINES, and lines 1→3 enclose two tracks", "Column 3 only", "None — the slash makes it invalid"],
          answer: 1, explain: "A 3-column grid has four vertical lines (1–4). Start line 1 to end line 3 wraps exactly columns 1 and 2." },
        { q: "Why is `repeat(auto-fit, minmax(150px, 1fr))` nicknamed the magic gallery line?",
          choices: ["It loads images automatically", "It only works with exactly 150 tiles", "It needs a media query for each screen size", "The browser fits as many ≥150px columns as the width allows — responsive with zero media queries"],
          answer: 3, explain: "auto-fit recomputes the column count at every width, and minmax keeps each column between 150px and an equal share." },
        { q: "You wrote `grid-template-areas: \"header header\" \"sidebar main\"` on the container. How does a child claim the sidebar spot?",
          choices: ["`grid-area: sidebar;` on that child", "`position: sidebar;` on that child", "`grid-template: sidebar;` on the container", "Give it the class .sidebar — Grid matches class names automatically"],
          answer: 0, explain: "Area names are connected by grid-area on the child. Class names mean nothing to grid-template-areas." },
        { q: "What layout does this produce?",
          code: ".app {\n  display: grid;\n  grid-template-columns: 200px 1fr;\n}",
          lang: "css",
          choices: ["Two equal columns", "One fixed 200px column, then a column that takes ALL the remaining space", "A single 201px-wide column", "200 columns, each 1fr wide"],
          answer: 1, explain: "Fixed and flexible tracks mix freely: the px track stays put while the fr track absorbs whatever width is left." },
        { q: "In `minmax(150px, 1fr)`, what does the browser promise about the track?",
          choices: ["It is always exactly 150px", "It can shrink all the way to 0", "It never goes below 150px and can grow to take an equal share of free space", "It is at most 150px"],
          answer: 2, explain: "minmax(min, max) is a floor and a ceiling: 150px is the smallest allowed size, 1fr the stretchy upper bound." }
      ]
    }
  ]
});
