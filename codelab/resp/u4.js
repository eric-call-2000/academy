...

    {
      ...
      questions: [
        ...
        { q: "The mobile-first recipe is…",
          choices: ["Build the desktop layout first, then shrink it with max-width queries", "Ship a separate m.example.com site for phones", "Write phone styles first, then layer on min-width queries", "Put every style inside its own media query"],
          answer: 2, explain: "The unqueried defaults serve the smallest screens; each @media (min-width: …) block layers on an enhancement for bigger ones. Nothing gets undone — only added." },
        { q: "The viewport is 1000px wide. How many columns does .cards get?",
          code: "@media (min-width: 700px) {\n  .cards { grid-template-columns: repeat(2, 1fr); }\n}\n@media (min-width: 900px) {\n  .cards { grid-template-columns: repeat(4, 1fr); }\n}",
          lang: "css",
          choices: ["2 — the first matching query wins and stops", "4 — both queries match, and the later rule wins", "6 — the two queries add their columns up", "An error — two queries can't set one property"],
          answer: 1, explain: "Media queries don't stop at the first match: at 1000px BOTH blocks apply, so the normal cascade decides and the later rule overrides. That is why mobile-first sheets order breakpoints small → large." },
        { q: "You want the ☰ menu button visible on phones only. Which CSS?",
          choices: ["Show .menuBtn by default; hide it inside a min-width query", "Set .menuBtn { display: none; } in the base styles", "Inside @media (min-width: 700px), set .menuBtn { display: block; }", "Delete the button from the HTML on wide screens"],
          answer: 0, explain: "Mobile-first: the default IS the phone behavior (the button is visible), and the wide-screen query takes it away with .menuBtn { display: none; } inside @media (min-width: 700px)." },
        { q: "Are breakpoints like 600, 768, 900 and 1200px official CSS values?",
          choices: ["Yes — browsers only accept those four widths", "Yes — any other number is silently ignored", "No — they're just widely copied conventions", "No — breakpoints must be written in rem, never px"],
