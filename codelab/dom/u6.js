...

    {
      ...
      questions: [
        ...
        { q: "What should render() usually do FIRST?",
          choices: ["Sort the state array in place", "Clear the container: list.innerHTML = \"\"", "Attach its event listeners again", "Save the state to localStorage"],
          answer: 1, explain: "Wipe, then rebuild from the array. Skip the wipe and every render() call appends a fresh set of duplicates." },
        { q: "Nothing ever gets removed. Why?",
          ...
          lang: "js",
          choices: ["filter() can't shrink an array", "You must use a for loop to remove items", "render() undoes the change", "dataset.id is the STRING \"2\", so t.id !== id is true even for the matching item"],
          answer: 3, explain: "dataset values are always strings, and 2 !== \"2\" under strict comparison. Convert first: Number(li.dataset.id)." },
        { q: "You type \"mil\" in a search box and only Milk stays visible. In a well-built filter, what actually changed?",
          choices: ["The items array — the others were deleted", "Only what render() drew — the array still holds everything", "The CSS file", "The HTML source on the server"],
          answer: 1, explain: "Filter the view, never the state. That's why clearing the search brings everything back instantly." },
        { q: "This sorts the list… and quietly breaks something. What?",
          code: "products.sort((a, b) => a.price - b.price);\nrender();",
          lang: "js",
          choices: ["It sorts descending by mistake", "sort() doesn't accept a comparator function", "sort() MUTATES — the state array itself is now permanently reordered", "Numbers can't be compared with minus"],
          answer: 2, explain: "sort() rearranges the array in place. Sort a copy — [...products].sort(...) — so your state keeps its original order." },
        { q: "What does [...products] give you?",
          choices: ["A new array holding the same items — a shallow copy", "A deep clone of every object inside", "A sorted version of products", "One long string of all the items"],
          answer: 0, explain: "Spread copies the array container — perfect for safe sorting. The objects inside are still shared; a deep clone takes more work." }
      ]
    }
  ]
});
