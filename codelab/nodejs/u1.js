...
window.CODELAB.addUnit("nodejs", {
  ...
  lessons: [

    {
      ...
      solution: {
        "script.js": "// Simulated Node.js runtime\nconst runtime = {\n  callStack: [],\n  taskQueue: [],\n  running: false,\n\n  run(task) {\n    this.callStack.push(task);\n    if (!this.running) {\n      this.running = true;\n      this.eventLoop();\n    }\n  },\n\n  setTimeout(callback, delay) {\n    // The real timer only ENQUEUES. When it fires it also wakes the loop —\n    // that wake-up is what makes a queued callback eventually run.\n    setTimeout(() => {\n      this.taskQueue.push(callback);\n      if (!this.running) {\n        this.running = true;\n        this.eventLoop();\n      }\n    }, delay);\n  },\n\n  eventLoop() {\n    while (this.callStack.length > 0) {\n      this.callStack.shift()();\n    }\n    if (this.taskQueue.length > 0) {\n      this.callStack.push(...this.taskQueue);\n      this.taskQueue = [];\n      this.eventLoop();\n    } else {\n      this.running = false;\n    }\n  }\n};\n\n// Test it\nruntime.run(() => console.log('sync'));\nruntime.setTimeout(() => console.log('async'), 0);\n"
      }
    },
...

    {
      ...
      steps: [
        { text: "Create a `heavyWork(item)` function that simulates CPU work and returns the count.",
          test: "T.expect(typeof heavyWork === 'function', 'Define heavyWork(item) function');\nconst did = heavyWork('test');\nT.expect(T.logged('processed test'), 'heavyWork should report the item it processed');\nT.expect(did > 0, 'heavyWork should return the work it did');" },
...

    {
      ...
      steps: [
        { text: "Create `asyncOperation(callback)` that simulates async work with 50% failure rate.",
          test: "T.expect(typeof asyncOperation === 'function', 'Define asyncOperation(callback)');\nlet called = false;\nasyncOperation(() => { called = true; });\nT.expect(!called, 'The callback must be ASYNC — it cannot have run yet');\nawait T.sleep(80);\nT.expect(called, 'Callback should be called once the timer fires');" },
        { text: "Callback should receive `(err, result)` — err on failure, result on success.",
          test: "let err, result;\nasyncOperation((e, r) => { err = e; result = r; });\nawait T.sleep(80);\nT.expect(err === null || err instanceof Error, 'First arg should be an Error or null');\nT.expect((err && result === null) || (!err && typeof result === 'string'),\n  'Exactly one of error / result should be filled in');" },
        { text: "Create `safeCallback(fn)` wrapper that catches errors and logs them.",
          test: "T.expect(typeof safeCallback === 'function', 'Define safeCallback(fn) wrapper');\nconst wrapped = safeCallback(() => { throw new Error('boom'); });\nT.expect(typeof wrapped === 'function', 'safeCallback should RETURN a function');\nwrapped();\nT.expect(T.logged('Callback threw'), 'A throwing callback should be caught and logged');" },
        { text: "Create `promiseOperation()` that returns a Promise, rejects on failure.",
          test: "T.expect(typeof promiseOperation === 'function', 'Define promiseOperation() returning Promise');\nconst p = promiseOperation();\nT.expect(p instanceof Promise, 'Should return a Promise');\nconst settled = await p.then(() => 'resolved', () => 'rejected');\nT.expect(settled === 'resolved' || settled === 'rejected', 'The promise should settle');" },
        { text: "Create `safePromise(p)` wrapper that catches and logs unhandled rejections.",
          test: "T.expect(typeof safePromise === 'function', 'Define safePromise(p) wrapper');\nconst guarded = safePromise(Promise.reject(new Error('nope')));\nT.expect(guarded && typeof guarded.then === 'function',\n  'safePromise must RETURN the chained promise, or the caller cannot use it');\nawait guarded;\nT.expect(T.logged('Unhandled rejection'), 'The rejection should be caught and logged');" }
      ],
      files: [
        { name: "script.js", content: "// Simulated async operation — alternates success and failure\nlet attempt = 0;\nfunction asyncOperation(callback) {\n  // Simulate async work with setTimeout (30ms)\n  // Odd attempts fail:    callback(new Error('failed'), null)\n  // Even attempts succeed: callback(null, 'success')\n}\n\n// Wrapper to make callbacks safe from throwing\nfunction safeCallback(fn) {\n  // Return a function that calls fn inside try/catch and logs 'Callback threw:'\n}\n\n// Promise version of async operation\nfunction promiseOperation() {\n  // Return a Promise that wraps asyncOperation\n}\n\n// Wrapper to handle unhandled rejections\nfunction safePromise(promise) {\n  // Attach .catch() to prevent an unhandled rejection — and RETURN the chain\n}\n\nconsole.log('Testing callback pattern...');\nasyncOperation((err, result) => {\n  if (err) console.error('Callback error:', err.message);\n  else console.log('Callback result:', result);\n});\n" }
      ],
      hints: [
      ...
      solution: {
        "script.js": "// Simulated async operation — alternates success and failure.\n// Deterministic on purpose: a coin flip here would make every checkpoint\n// below flaky, and the lesson is about the two code paths, not about luck.\nlet attempt = 0;\nfunction asyncOperation(callback) {\n  setTimeout(() => {\n    attempt++;\n    const success = attempt % 2 === 0;\n    if (success) {\n      callback(null, 'success');\n    } else {\n      callback(new Error('failed'), null);\n    }\n  }, 30);\n}\n\n// Wrapper to make callbacks safe from throwing\nfunction safeCallback(fn) {\n  return (...args) => {\n    try {\n      fn(...args);\n    } catch (err) {\n      console.error('Callback threw:', err.message);\n    }\n  };\n}\n\n// Promise version of async operation\nfunction promiseOperation() {\n  return new Promise((resolve, reject) => {\n    asyncOperation((err, result) => {\n      if (err) reject(err);\n      else resolve(result);\n    });\n  });\n}\n\n// Wrapper to handle unhandled rejections. It must RETURN the chained promise:\n// forget that and the caller's .then() runs on undefined.\nfunction safePromise(promise) {\n  return promise.catch(err => {\n    console.error('Unhandled rejection:', err.message);\n    return null;\n  });\n}\n\nconsole.log('Testing callback pattern...');\nasyncOperation((err, result) => {\n  if (err) console.error('Callback error:', err.message);\n  else console.log('Callback result:', result);\n});\n\nconsole.log('Testing promise pattern...');\nsafePromise(promiseOperation())\n  .then(result => console.log('Promise result:', result));\n"
      }
    },

    {
      ...
      solution: {
        "script.js": "// Universal defer function supporting different timing methods.\n// The ordering these produce is the lesson: sync runs now, nextTick drains\n// before anything else deferred, setImmediate goes ahead of a 0ms timer.\nfunction defer(fn, method = 'setImmediate') {\n  switch (method) {\n    case 'sync':\n      fn();\n      break;\n    case 'nextTick':\n      process.nextTick(fn);\n      break;\n    case 'setImmediate':\n      setImmediate(fn);\n      break;\n    case 'setTimeout':\n      setTimeout(fn, 0);\n      break;\n    default:\n      setImmediate(fn);\n  }\n}\n\n// Test the ordering\nconsole.log('1. sync start');\n\ndefer(() => console.log('2. nextTick'), 'nextTick');\n\ndefer(() => console.log('3. setImmediate'), 'setImmediate');\n\ndefer(() => console.log('4. setTimeout'), 'setTimeout');\n\nconsole.log('5. sync end');\n\n// Expected order: 1, 5, 2, 3, 4\n"
      }
    },

    {
      ...
      questions: [
        ...
        { q: "Which operation BLOCKS the event loop?",
          choices: ["fs.readFile() with a callback", "http.request()", "A while(true) loop", "setTimeout()"],
        ...
        { q: "How do you handle errors in a callback?",
          choices: ["Wrap the callback body in try/catch and rethrow", "First argument is error, check if (err)", "The callback throws on error", "Callbacks can't have errors"],
        ...
        { q: "What happens if you don't catch a Promise rejection?",
          choices: ["Nothing, it's silently ignored", "It throws synchronously in the enclosing try/catch block", "Node.js logs a warning (or crashes in older versions)", "The promise retries automatically"],
