...
window.CODELAB.addUnit("nodejs", {
  ...
  lessons: [
...

    {
      ...
      files: [
        { name: "script.js", content: "// Custom error classes\nclass AppError extends Error {\n  constructor(message, statusCode = 500, errorCode = 'INTERNAL_ERROR') {\n    super(message);\n    this.name = this.constructor.name;\n    // TODO: keep statusCode and errorCode on the error, and mark it\n    // operational — an expected failure, not a programmer bug.\n  }\n}\n\nclass NotFoundError extends AppError {\n  constructor(resource) {\n    // TODO: call super() with a \"<resource> not found\" message,\n    // status 404 and code 'NOT_FOUND', then remember the resource.\n    super(resource);\n  }\n}\n\nclass ValidationError extends AppError {\n  constructor(field, message) {\n    // TODO: call super() with a validation message, status 400 and\n    // code 'VALIDATION_ERROR', then remember which field failed.\n    super(message);\n  }\n}\n\n// Try them out\nconst missing = new NotFoundError('User');\nconsole.log(missing.message, missing.statusCode, missing.errorCode);\n" }
      ],
      hints: [
...

    {
      ...
      kind: "js", chip: "NODE", xp: 15, mins: 12,
      brief: "Debugging Node.js goes beyond console.log. Key techniques:\n\n- **node inspect** — CLI mockDebugger with breakpoints, stepping\n- **Chrome DevTools** — graphical debugging with `node --inspect`\n- **mockDebugger statement** — programmatic breakpoint\n- **--inspect-brk** — pause on first line\n- **NODE_DEBUG** — debug specific modules\n\nChrome DevTools is the most powerful — set breakpoints, inspect variables, step through code.",
      example: { lang: "js", code: "// app.js\nfunction buggyFunction(arr) {\n  mockDebugger; // breakpoint here\n  return arr.map(x => x * 2).filter(x => x > 10);\n}\n\n// Run with: node --inspect-brk app.js\n// Then open chrome://inspect in Chrome\n\n// Or use CLI mockDebugger\n// $ node inspect app.js\n// < mockDebugger listening on ws://127.0.0.1:9229\n// debug> cont\n// debug> next\n// debug> repl\n// debug> quit" },
      steps: [
        { text: "Create a mock debugger object with breakpoint support.",
          test: "T.expect(typeof mockDebugger === 'object', 'Create mockDebugger object');\nT.expect(typeof mockDebugger.setBreakpoint === 'function', 'Should have setBreakpoint method');" },
        { text: "Implement step execution: step into, step over, step out.",
          test: "T.expect(typeof mockDebugger.stepInto === 'function', 'Should have stepInto method');\nT.expect(typeof mockDebugger.stepOver === 'function', 'Should have stepOver method');\nT.expect(typeof mockDebugger.stepOut === 'function', 'Should have stepOut method');" },
        { text: "Add variable inspection at breakpoints.",
          test: "T.expect(typeof mockDebugger.inspect === 'function', 'Should have inspect method');\nconst context = { x: 5, y: 10 };\nT.eq(mockDebugger.inspect('x', context), 5, 'Should inspect variable in context');" },
        { text: "Simulate debugging a buggy function with breakpoints.",
          test: "mockDebugger.setBreakpoint('buggyFunction', 1);\nconst result = buggyFunction([1, 2, 3, 4, 5]);\nT.expect(T.logged('Breakpoint hit'), 'Should hit breakpoint');" }
      ],
      files: [
        { name: "script.js", content: "// Mock debugger for debugging techniques\nconst mockDebugger = {\n  breakpoints: new Map(),\n  callStack: [],\n  currentContext: {},\n\n  setBreakpoint(functionName, lineNumber) {\n    console.log(`Breakpoint set at ${functionName}:${lineNumber}`);\n    this.breakpoints.set(`${functionName}:${lineNumber}`, true);\n  },\n\n  hitBreakpoint(functionName, lineNumber, context) {\n    // TODO: if a breakpoint is registered for this function and line, log\n    // 'Breakpoint hit at <fn>:<line>', remember `context` as the current\n    // scope, push a frame onto callStack, and return true. Otherwise false.\n  },\n\n  stepInto() {\n    console.log('Stepping into function...');\n  },\n\n  stepOver() {\n    console.log('Stepping over line...');\n  },\n\n  stepOut() {\n    console.log('Stepping out of function...');\n  },\n\n  inspect(variableName, context) {\n    // TODO: look the variable up in `context` if one was passed, otherwise in\n    // this.currentContext. Log and return its value, or log that it is not\n    // defined and return undefined.\n  },\n\n  continue() {\n    console.log('Continuing execution...');\n  },\n\n  getCallStack() {\n    return this.callStack;\n  }\n};\n\n// Buggy function to debug\nfunction buggyFunction(arr) {\n  let result = [];\n\n  if (mockDebugger.hitBreakpoint('buggyFunction', 1, { arr, result })) {\n    mockDebugger.inspect('arr');\n    mockDebugger.stepOver();\n  }\n\n  for (let i = 0; i < arr.length; i++) {\n    const doubled = arr[i] * 2;\n\n    if (mockDebugger.hitBreakpoint('buggyFunction', 2, { i, arr, doubled })) {\n      mockDebugger.inspect('i');\n      mockDebugger.inspect('doubled');\n    }\n\n    if (doubled > 10) {\n      result.push(doubled);\n    }\n  }\n\n  return result;\n}\n\n// Set breakpoints and debug\nconsole.log('=== Starting debugging session ===');\nmockDebugger.setBreakpoint('buggyFunction', 1);\nmockDebugger.setBreakpoint('buggyFunction', 2);\n\nconsole.log('\\nCalling buggyFunction...');\nconst input = [1, 2, 3, 4, 5, 6, 7];\nconst output = buggyFunction(input);\n\nconsole.log('Input:', input);\nconsole.log('Output:', output);\n" }
      ],
      hints: [
        "Create a mockDebugger object that tracks breakpoints, call stack, and current execution context",
        "When a breakpoint is hit, pause execution and allow inspection of variables",
        "Support basic mockDebugger commands: stepInto, stepOver, stepOut, continue, inspect"
      ],
      solution: {
        "script.js": "// Mock mockDebugger for debugging techniques\n\nconst mockDebugger = {\n  breakpoints: new Map(),\n  callStack: [],\n  currentContext: {},\n\n  setBreakpoint(functionName, lineNumber) {\n    console.log(`Breakpoint set at ${functionName}:${lineNumber}`);\n    this.breakpoints.set(`${functionName}:${lineNumber}`, true);\n  },\n\n  hitBreakpoint(functionName, lineNumber, context) {\n    const key = `${functionName}:${lineNumber}`;\n    if (this.breakpoints.has(key)) {\n      console.log(`\\n=== Breakpoint hit at ${functionName}:${lineNumber} ===`);\n      this.currentContext = context;\n      this.callStack.push({ function: functionName, line: lineNumber, context });\n      return true;\n    }\n    return false;\n  },\n\n  stepInto() {\n    console.log('Stepping into function...');\n  },\n\n  stepOver() {\n    console.log('Stepping over line...');\n  },\n\n  stepOut() {\n    console.log('Stepping out of function...');\n  },\n\n  inspect(variableName, context) {\n    const scope = context || this.currentContext;    if (variableName in scope) {\n      console.log(`${variableName} =`, scope[variableName]);\n      return scope[variableName];\n    }\n    console.log(`${variableName} is not defined in current context`);\n    return undefined;\n  },\n\n  continue() {\n    console.log('Continuing execution...');\n  },\n\n  getCallStack() {\n    return this.callStack;\n  }\n};\n\n// Buggy function to debug\nfunction buggyFunction(arr) {\n  let result = [];\n  \n  if (mockDebugger.hitBreakpoint('buggyFunction', 1, { arr, result })) {\n    mockDebugger.inspect('arr');\n    mockDebugger.stepOver();\n  }\n  \n  for (let i = 0; i < arr.length; i++) {\n    const doubled = arr[i] * 2;\n    \n    if (mockDebugger.hitBreakpoint('buggyFunction', 2, { i, arr, doubled })) {\n      mockDebugger.inspect('i');\n      mockDebugger.inspect('doubled');\n    }\n    \n    if (doubled > 10) {\n      result.push(doubled);\n    }\n  }\n  \n  return result;\n}\n\n// Set breakpoints and debug\nconsole.log('=== Starting debugging session ===');\nmockDebugger.setBreakpoint('buggyFunction', 1);\nmockDebugger.setBreakpoint('buggyFunction', 2);\n\nconsole.log('\\nCalling buggyFunction...');\nconst input = [1, 2, 3, 4, 5, 6, 7];\nconst output = buggyFunction(input);\n\nconsole.log('\\n=== Debugging session complete ===');\nconsole.log('Input:', input);\nconsole.log('Output:', output);\nconsole.log('\\nCall stack:', mockDebugger.getCallStack());\n"
      }
    },

    {
      ...
      steps: [
        ...
        { text: "Add log level filtering (only show logs at or above set level).",
          test: "logger.level = 'warn';\nlogger.info('quiet-info-probe');\nlogger.warn('loud-warn-probe');\nT.expect(!T.logged('quiet-info-probe'), 'Should filter info logs when level is warn');\nT.expect(T.logged('loud-warn-probe'), 'Should show warn logs');\nlogger.level = 'info';" }
      ],
      files: [
        { name: "script.js", content: "// Structured logger\nconst logger = {\n  level: 'info',\n  levels: { trace: 0, debug: 1, info: 2, warn: 3, error: 4 },\n\n  log(level, context, message) {\n    // TODO: accept BOTH log(level, 'message') and log(level, {ctx}, 'message');\n    // drop the entry if `level` is below this.level; otherwise print one JSON\n    // line carrying timestamp, level, message and any context fields.\n  },\n\n  trace(context, message) { this.log('trace', context, message); },\n  debug(context, message) { this.log('debug', context, message); },\n  info(context, message) { this.log('info', context, message); },\n  warn(context, message) { this.log('warn', context, message); },\n  error(context, message) { this.log('error', context, message); },\n\n  setLevel(level) {\n    if (level in this.levels) {\n      this.level = level;\n      console.log(`Log level set to: ${level}`);\n    }\n  }\n};\n\n// Test the logger\nconsole.log('=== Testing structured logger ===');\nlogger.info('service starting');\nlogger.info({ userId: 42 }, 'user signed in');\n" }
      ],
      hints: [
      ...
      solution: {
        "script.js": "// Structured logger\nconst logger = {\n  level: 'info',\n  levels: { trace: 0, debug: 1, info: 2, warn: 3, error: 4 },\n\n  log(level, context, message) {\n    // Called either as log(level, 'message') or log(level, {ctx}, 'message').\n    // Supporting both is why real loggers feel natural to use.\n    if (message === undefined) {\n      message = context;\n      context = {};\n    }\n    if (context === null || typeof context !== 'object') {\n      context = {};\n    }\n    if (this.levels[level] < this.levels[this.level]) {\n      return;\n    }\n    const logEntry = {\n      timestamp: new Date().toISOString(),\n      level: level,\n      message: message,\n      ...context\n    };\n    console.log(JSON.stringify(logEntry));\n  },\n\n  trace(context, message) { this.log('trace', context, message); },\n  debug(context, message) { this.log('debug', context, message); },\n  info(context, message) { this.log('info', context, message); },\n  warn(context, message) { this.log('warn', context, message); },\n  error(context, message) { this.log('error', context, message); },\n\n  setLevel(level) {\n    if (level in this.levels) {\n      this.level = level;\n      console.log(`Log level set to: ${level}`);\n    }\n  }\n};\n\n// Test the logger\nconsole.log('=== Testing structured logger ===');\nlogger.info('service starting');\nlogger.info({ userId: 42 }, 'user signed in');\nlogger.error({ err: 'ETIMEDOUT' }, 'upstream call failed');\n"
      }
    },
