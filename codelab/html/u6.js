...

    {
      ...
      questions: [
        ...
        { q: "The text between `<video>` and `</video>` is…",
          choices: ["Fallback for unsupported browsers", "The caption shown under the player", "The title shown in the player bar", "Required by the HTML5 video spec"],
          answer: 0, explain: "Old or limited browsers render the inner content instead of the player — a graceful degradation, usually a \"your browser can't play this\" line plus a download link. It's entirely optional, and browsers that **can** play the video never show it, which is why a caption or title doesn't belong there." },
        { q: "Why must every `<iframe>` have a `title` attribute?",
          choices: ["Screen readers announce it by title", "It sets the embedded page's tab name", "The src won't load without it", "It makes the frame resize responsively"],
          answer: 0, explain: "\"Embedded map of our office\" beats \"frame\" — that's what assistive tech reads out when a user tabs into the iframe. Leave the title off and they hear a nameless \"frame\" with no clue what's inside. The frame still loads and still sizes exactly the same; only the announcement changes." },
        ...
        { q: "How do you display a literal `<p>` as text on a page?",
          ...
          lang: "html",
          choices: ["&lt;p&gt;", "<p>", "\\<p\\>", "«p»"],
          answer: 0, explain: "&lt; and &gt; are the angle-bracket entities, and the browser prints them as < and > instead of reading a tag. A raw <p> would start an actual paragraph, backslashes are not an escape character in HTML, and « » are just quotation marks." },
