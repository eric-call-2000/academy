...

    {
      ...
      questions: [
        { q: "Why prefer `<strong>` over `<b>`?",
          choices: ["<strong> means important; <b> is just bold", "<b> is deprecated and browsers ignore it", "<strong> renders bolder text than <b> does", "<strong> italicizes as well as bolds"],
          answer: 0, explain: "Both render identically — bold, same weight — but <strong> tells assistive tech (and search engines) that the content matters, so a screen reader can announce it with emphasis. <b> is still valid HTML; it just changes pixels and means nothing." },
        ...
        { q: "How do you link to a section **on the same page**?",
          ...
          lang: "html",
          choices: ["href=\"#pricing\" plus id=\"pricing\"", "href=\"pricing.html\" on the link", "class=\"pricing\" on both elements", "name=\"pricing\" on both elements"],
          answer: 0, explain: "The # prefix means \"find the element with this id **on this page**\" — so the link carries `href=\"#pricing\"` and the target heading carries `id=\"pricing\"`. No matching id, no jump. (`name` anchors are the deprecated 1990s way; classes are for styling, never link targets.)" },
        { q: "The `alt` attribute on `<img>` is…",
          choices: ["A text description of the image", "A fallback image URL if the first fails", "A caption displayed under the image", "The tooltip text shown on hover"],
          answer: 0, explain: "alt is the accessibility backbone of images: screen readers speak it, and browsers print it in the broken-image box when the file never loads. (<figcaption> is the visible caption; `title` is the hover tooltip.)" },
        ...
        { q: "Where does a **nested** sub-list go?",
          choices: ["Inside the parent <li>, before </li>", "Opened inside the <li>, closed after it", "Directly inside the outer <ul>", "After the parent list closes"],
          answer: 0, explain: "<li>Parent <ul>…sub items…</ul></li> — the sub-list belongs to its parent item, so it opens **and** closes inside that <li>. A <ul> dropped straight into another <ul> is invalid markup, one left open past </li> overlaps the tags, and one placed after the outer list ends isn't nested at all." },
