...

    {
      ...
      questions: [
        ...
        { q: "A blog post in a feed — which wrapper fits best?",
          choices: ["`<article>` — it's self-contained and would make sense on its own", "`<aside>`", "`<div>`", "`<nav>`"],
          answer: 0, explain: "The syndication test: could it stand alone elsewhere? Then article." },
        { q: "The right alt text for a decorative flourish image is…",
          choices: ["alt=\"\" — empty, so screen readers skip it", "alt=\"image\"", "No alt attribute at all", "alt=\"decorative flourish graphic ornament\""],
          answer: 0, explain: "Empty alt = deliberate skip. MISSING alt makes screen readers read the filename — the worst outcome." },
        { q: "Why is `<div onclick=…>` worse than `<button>`?",
          choices: ["Divs can't be keyboard-focused or announced as pressable — buttons get that for free", "Divs are slower", "Buttons look better", "It isn't worse"],
          answer: 0, explain: "Real buttons ship with focus, Enter/Space activation, and screen-reader semantics built in." },
        ...
        { q: "What does `aria-label=\"Close\"` on a ✕ button do?",
          choices: ["Gives screen readers a spoken name for an icon-only control", "Closes the window", "Adds a tooltip for everyone", "Styles the button"],
          answer: 0, explain: "Visually it's a ✕; aurally it becomes \"Close, button\". Icon-only controls always need this." },
        { q: "Which meta tag makes a page render properly on phones?",
          choices: ["`<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">`", "`<meta charset=\"UTF-8\">`", "`<meta name=\"mobile\" content=\"true\">`", "`<title>mobile</title>`"],
          answer: 0, explain: "Without the viewport meta, phones pretend to be 980px-wide desktops and shrink everything." }
      ]
    }
  ]
});
