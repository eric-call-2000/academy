...

    {
      ...
      questions: [
        { q: "What connects a `<label>` to its input?",
          choices: ["label's for matches the input's id", "They must be adjacent", "label's name matches input's name", "A shared class"],
          answer: 0, explain: "for=\"email\" ↔ id=\"email\". Clicking the label then focuses the field — huge on touch screens." },
        { q: "Why is a placeholder **not** a substitute for a label?",
          choices: ["It disappears when the user types, leaving the field unexplained", "Placeholders are invisible", "Placeholders can't contain text", "It submits with the form"],
          answer: 0, explain: "Mid-form, users forget what a half-filled field was. Labels stay visible; placeholders are hints only." },
        { q: "What makes radio buttons act as one choose-one group?",
          choices: ["Sharing the same name attribute", "Sharing the same id", "Being inside one div", "Using type=\"group\""],
          answer: 0, explain: "name=\"size\" on all of them = one group, one selection. ids must still be unique for labels." },
        { q: "When should you use a checkbox instead of radios?",
          choices: ["When each choice is independent on/off (pick many)", "When only one option is allowed", "For submitting the form", "For passwords"],
          answer: 0, explain: "Checkbox = any number can be on. Radio = exactly one of the group." },
        { q: "What does this input guarantee?",
          code: "<input type=\"number\" required min=\"13\" max=\"120\">",
          lang: "html",
          choices: ["The browser blocks submit unless it holds a number from 13 to 120", "It converts text to numbers", "It's optional but numeric", "Nothing without JavaScript"],
          answer: 0, explain: "required + min/max = built-in validation, before any JS runs. (Servers still re-check — never trust the client alone!)" },
        ...
        { q: "`type=\"email\"` on a phone gives users…",
          choices: ["An email-optimized keyboard (with @) plus built-in format checking", "Auto-sent emails", "A dropdown of their contacts", "Nothing different"],
          answer: 0, explain: "Input types adapt the mobile keyboard AND add validation — free UX wins." }
      ]
    },
