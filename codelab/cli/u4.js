...
window.CODELAB.addUnit("cli", {
  ...
  cheat: [
    { h: "Redirecting output", lang: "sh", code: "ls > files.txt        # write (replaces the file entirely)\nls >> files.txt       # append (adds to the end)\ndate > /dev/null      # throw the output away, keep the exit code", note: "`>` truncates first and then writes — even if the command fails, so a failed command leaves you an EMPTY file, not the old one. When you meant to add, `>>` is the one you wanted." },
